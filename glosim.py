...
#===============================================================================================================================================
        ...
        if (nprocs<=1):
            ...
            def dochunk(psim, iproc, nprocs):
                for iframe in range(iproc, nf, nprocs):
                    sli=sl[iframe]
                    sys.stderr.write("Matrix row (process %2d) %5d    \n" % (iproc, iframe))
                    for jframe in range(0,iframe):
...
if __name__ == '__main__':
      ...
      #cProfile.runctx( "main(args.filename, nd=args.n, ld=args.l, coff=args.c, cotw=args.cotw, gs=args.g, mu=args.mu, centerweight=args.cw, periodic=args.periodic, usekit=args.usekit, kit=args.kit,alchemyrules=args.alchemy_rules, kmode=args.kernel, nonorm=args.nonorm, permanenteps=args.permanenteps, reggamma=args.gamma, noatom=noatom, nocenter=nocenter, envsim=args.envsim, nprocs=args.np, verbose=args.verbose, envij=envij, prefix=args.prefix, nlandmark=args.nlandmarks, printsim=args.distance,ref_xyz=args.refxyz,partialsim=args.livek,lowmem=args.lowmem,restartflag=args.restart, zeta=args.zeta, xspecies=args.separate_species, alrange=(args.first,args.last), refrange=(args.reffirst, args.reflast))", globals(), locals(), filename="glosim.profile")
